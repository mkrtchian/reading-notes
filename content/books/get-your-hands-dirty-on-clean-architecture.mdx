...
- L'**architecture** représente la **structure** du système logiciel.
...
- Comment être sûr qu'on conserve la maintenabilité ? Au moins en s'assurant qu'on choisit et conserve une architecture qui la permet.
  - La clean archi en est une possibilité.
...
- On finit en général par créer des **Bounded Contexts** (BC) pour séparer certaines parties de la codebase des autres,. C'est utile entre autres pour leur permettre d'évoluer indépendamment, et pour garder des morceaux de code indépendants qui peuvent rentrer dans la mémoire immédiate d'un être humain.
- La solution la plus immédiate pour gérer plusieurs BCs en clean architecture est de faire les choses de manière **bien découplée** :
  - On crée :
    - Un _output port_ sur le core du BC appelant.
    - Un _input port_ sur le core du BC appelé.
    - Un _output adapter_ (qu'on pourrait appeler _Domain Adapter_) qui implémente l'_output port_, et appelle l'_input port_.
...
- Dans l'article original d'Alistair Cockburn sur l'hexagonal architecture, il n'y a pas de mention de _Bounded Context_ mais seulement d'_Application_ pour ce qui est du core d'un hexagone.
  - L'auteur en déduit qu'on peut considérer que **les BCs peuvent exister à l'intérieur d'un même hexagone**.
  - On se retrouve avec un _input adapter_ web, plusieurs _use-cases_ à l'intérieur du core, et un _output adapter_ de persistance.
    - L'auteur dit qu'on peut avoir un ou plusieurs _input ports_ pour chaque BC selon les cas.
    - Pour les _output ports_ de persistance, il dit qu'il est préférable de ne pas les partager entre BCs, parce qu'on ne veut pas que les BCs partagent la DB et aient le même data model. Ils doivent au moins avoir des schémas différents, ou au minimum des tables différentes.
  ...
  - Pour faire communiquer les BCs entre eux, l'auteur propose deux solutions tirées du DDD, et nous propose de **lire la littérature DDD** pour en savoir plus.
    - 1 - Un _use-case_ qui a besoin d'une info venant d'un _use-case_ d'un autre BC peut consommer des **domain events** que l'autre émet, les stocker en base chez lui, et les réutiliser ensuite.
    - 2 - On peut avoir un **application service** à l'intérieur de l'hexagone qui coordonne les appels aux _use-cases_ de chaque BC (qui sont appelés _Domain Service_).
      - Dans ce cas précis, on peut utiliser l'_application service_ extérieur aux BCs pour porter une transaction DB, qui pourrait être utile pour l'ensemble des BCs mis en jeu.
      - NDLR : déjà que c'est chaud d'envisager une transaction pour plusieurs aggregates, comment on fait pour envisager une transaction pour plusieurs use-cases dans plusieurs BCs différents ?

## 14 - A Component-Based Approach to Software Architecture

- La **component-based architecture** peut être utile :
  - **En complément de l'hexagonal architecture** dans le cas où on choisit d'avoir un hexagone pour plusieurs _bounded contexts_, mais qu'on veut faire apparaître les limites entre chaque _bounded context_ à l'intérieur du _core_.
...
- La modularité permet de construire des systèmes complexes.
  - Elle nécessite d'avoir des modules (l'auteur préfère le terme _component_) qui :
    - **1 - Encapsulent des détails d'implémentation.**
    - **2 - Exposent une interface.**
  - Un _component_ peut contenir des _sub-components_ qui encapsulent eux-aussi de la logique et exposent une API. Leur API ne sera accessible qu'à l'intérieur du composant qui les contient.
...
- L'auteur présente un exemple de projet utilisant cette architecture : il s'agit d'un logiciel faisant du scraping de pages web, de l'analyse sur ce qui a été récolté, et stocke ensuite ça dans une DB.
  - Le logiciel est un _component_ en lui-même puisqu'il présente une API publique et cache le reste.
  - Il contient 3 sub-components :
    - Un _component_ servant d'_input adapter_ consommant une queue.
    - Un _component_ contenant la logique business pour analyser les données des pages et sortir les résultats à stocker.
    - Un _component_ servant d'_output adapter_ de persistance, pour stocker le résultat en DB.
- Il ne faut pas oublier d'ajouter des contraintes pour garantir l'architecture (avec des outils comme ArchUnit). Les règles sont très simples :
  - Aucun code extérieur à un _component_ ne doit accéder au code à l'intérieur de celui-ci. Il ne peut accéder qu'à son API publique.
  - Quand un _component_ est à l'intérieur d'un autre _component_, il fait partie des détails d'implémentation de celui-ci.
